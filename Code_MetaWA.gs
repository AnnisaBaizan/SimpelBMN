...
const CONFIG = {
  ...
  // --- WhatsApp via Meta Cloud API (https://developers.facebook.com) ---
  WA_PHONE_NUMBER_ID : 'GANTI_PHONE_NUMBER_ID',          // Phone Number ID dari Meta App Dashboard
  WA_ACCESS_TOKEN    : 'GANTI_ACCESS_TOKEN',             // Permanent Access Token dari Meta
  WA_TUJUAN          : '6281532250712',                  // Nomor penerima format 62xxx
  WA_TEMPLATE_NAME   : 'notif_bmn',                      // Nama template yang sudah disetujui Meta
  WA_TEMPLATE_LANG   : 'id',                             // Kode bahasa template (id = Indonesia)
...
// ============================================================
//  KIRIM NOTIFIKASI WHATSAPP via Meta WhatsApp Cloud API
//  Setup: https://developers.facebook.com/docs/whatsapp/cloud-api
//
//  PENTING: Meta wajib pakai template pesan yang sudah disetujui.
//  Template "notif_bmn" harus dibuat & disetujui dulu di Meta Business Manager.
//  Lihat panduan template di bawah fungsi ini.
// ============================================================
function kirimWhatsApp(d, fotoLinks) {
  try {
    const kondisiEmoji = d.kondisi === 'Rusak Berat' ? '🔴' : '🟡';

    // Nilai variabel yang dimasukkan ke template
    // Urutan harus sama persis dengan urutan {{1}}, {{2}}, dst di template Meta
    const templateParams = [
      d.nomor,                          // {{1}} Nomor Surat
      d.tanggalSurat,                   // {{2}} Tanggal
      d.nama,                           // {{3}} Nama Pengusul
      d.bagian,                         // {{4}} Unit/Bagian
      d.namaBarang,                     // {{5}} Nama Barang
      d.ruangan,                        // {{6}} Ruangan
      d.nup,                            // {{7}} NUP BMN
      kondisiEmoji + ' ' + d.kondisi,   // {{8}} Kondisi
      d.keluhan.substring(0, 1024),     // {{9}} Keluhan (max 1024 char)
    ];
    ...
    if (result.error) {
      Logger.log('WhatsApp Meta gagal: ' + JSON.stringify(result.error));
    } else {
      Logger.log('WhatsApp Meta terkirim, message_id: ' + result.messages[0].id);
    }
  } catch (e) {
    Logger.log('WhatsApp Meta error (non-fatal): ' + e.message);
  }
}

// ============================================================
//  TEMPLATE PESAN untuk Meta Business Manager
//  Buat template baru dengan nama "notif_bmn", bahasa Indonesia
//  Category: UTILITY | Body text:
//
...
//  {{9}}
//
//  _Pesan otomatis Sistem BMN Poltekkes Palembang_
// ============================================================
